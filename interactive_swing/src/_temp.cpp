...
#define FPS 10
#define ENDURANCE 40.0 // MEAN 5.4 DEGREE
...
float initial_pos_encoder, pos_encoder,prev_pos_encoder;
float getAverageRead(uint32_t anal_in, int num);
bool derivative_threshold_over(float threshold);
//SETUP
...
void loop()
{
  ...
  if (abs(num_of_steps_for_master - num_of_steps_for_encoder_pos) > ENDURANCE &&
      abs(num_of_steps_for_master - num_of_steps_for_encoder_pos) < (400 - ENDURANCE) && !(derivative_threshold_over(100)))
  {
...
bool derivative_threshold_over(float threshold){
  if(abs(pos_encoder-prev_pos_encoder) >threshold)
    return true;
  else
    return false;
